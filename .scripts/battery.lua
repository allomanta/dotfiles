...
function draw(cr)
	...
	cr:move_to(start + charge_width, BAR_Y)
	cr:set_line_width(4)
	cr:rel_line_to(width - charge_width - 1, 0)
	cr:stroke()

	-- End cap
	cr:move_to(start + width - 1, BAR_Y)
	cr:set_line_width(2)
	cr:rel_line_to(1, 0)
	cr:stroke()
	...
	-- Filled part
	cr:set_source_rgb(unpack(color))
	cr:move_to(start + 1, BAR_Y)
	cr:set_line_width(4)
	cr:rel_line_to(charge_width - 1, 0)
	cr:stroke()

	-- Start cap
	cr:move_to(start, BAR_Y)
	cr:set_line_width(2)
